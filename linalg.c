...
{
    int x = ((q[X] * q[X]) + (q[Y] * q[Y]) + (q[Z] * q[Z]) + (q[W] * q[W])) >>
            SCALE;
...
{
    out[W] = -FixedMulFixed(q[X], v[X]) - FixedMulFixed(q[Y], v[Y]) -
             FixedMulFixed(q[Z], v[Z]);
    out[X] = FixedMulFixed(q[W], v[X]) + FixedMulFixed(q[Y], v[Z]) -
             FixedMulFixed(q[Z], v[Y]);
    out[Y] = FixedMulFixed(q[W], v[Y]) + FixedMulFixed(q[Z], v[X]) -
             FixedMulFixed(q[X], v[Z]);
    out[Z] = FixedMulFixed(q[W], v[Z]) + FixedMulFixed(q[X], v[Y]) -
             FixedMulFixed(q[Y], v[X]);
}

void quat_mulquat(const quat qa, const quat qb, quat out)
{
    out[W] = FixedMulFixed(qa[W], qb[W]) - FixedMulFixed(qa[X], qb[X]) -
             FixedMulFixed(qa[Y], qb[Y]) - FixedMulFixed(qa[Z], qb[Z]);
    out[X] = FixedMulFixed(qa[X], qb[W]) + FixedMulFixed(qa[W], qb[X]) +
             FixedMulFixed(qa[Y], qb[Z]) - FixedMulFixed(qa[Z], qb[Y]);
    out[Y] = FixedMulFixed(qa[Y], qb[W]) + FixedMulFixed(qa[W], qb[Y]) +
             FixedMulFixed(qa[Z], qb[X]) - FixedMulFixed(qa[X], qb[Z]);
    out[Z] = FixedMulFixed(qa[Z], qb[W]) + FixedMulFixed(qa[W], qb[Z]) +
             FixedMulFixed(qa[X], qb[Y]) - FixedMulFixed(qa[Y], qb[X]);
}
